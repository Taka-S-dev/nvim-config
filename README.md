...
ヘッドレスの nvim がこの設定を読み込み、過去に実際に壊れた箇所を 15 秒ほどで確かめる: 手を付けていないファイルの全行に変更マークが付く、`:grep` が返ってこない、索引ファイルやバックアップが検索結果に混ざる、定義ジャンプの着地先、ピークの小窓に実ファイルが入り込む、`tags` の出来る場所、ctags の複数候補の表示、ステータスラインの表示が消えない、Markdown が整形されずに素のまま表示される、プロセスを延々と起動する。失敗した項目の数が終了コードになる。
